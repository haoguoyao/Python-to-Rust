...
def _detect_pathlib_path(p: io.TextIOWrapper):
...
def _ispath(p: io.TextIOWrapper) -> bool:
...
def _load_date(val: str) -> Union[datetime.date, datetime.datetime]:
    microsecond = 0
    tz = None
    try:
        if len(val) > 19:
...
class TomlDecoder(object):
    ...
    def __init__(self, _dict: Type[dict]=dict):
        self._dict = _dict
    ...
    def load_line(self, line: str, currentlevel: Dict[str, Any], multikey: None, multibackslash: bool) -> Tuple[str, str, bool]:
        i = 1
    ...
    def load_value(self, v: str, strictly_valid: bool=True) -> Any:
        if not v:
    ...
    def preserve_comment(self, line_no: int, key: str, comment: str, beginline: bool):
        pass
